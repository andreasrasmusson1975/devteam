...
instructions=f"""
Generate the content of the docker file. Analyze the provided architecture carefully to create
the appropriate content for the specified file.
...
def get_docker_file_user_prompt(
...
async def get_docker_file_content(
    user_prompt: str,
    system_prompt
) -> dict:
    try:
        ...
        validate_chat_response(response, get_docker_file_generation_output_schema())
        response_dict = json.loads(response)
        return response_dict
    except Exception as e:
        print(f"Error in get_docker_file_content: {e}")
        raise e